...
#include "mpi.h"
#include "omp.h"
#define STB_IMAGE_WRITE_IMPLEMENTATION // Quan trọng!
#include "stb_image_write.h"        // Thêm dòng này
...
int main(int argc, char** argv) {
	...
	char* image = NULL; // Khởi tạo thành NULL
	color_t imageType;
	/* MPI world topology */
	...
	// --- BẮT ĐẦU KHỐI CODE THU THẬP DỮ LIỆU VÀ GHI FILE PNG ---

	uint8_t* final_image_buffer = NULL;
	int image_channels = (imageType == GREY) ? 1 : 3;

	// Tính kích thước dữ liệu cục bộ (không gồm padding)
	size_t local_actual_data_bytes = (size_t)rows * cols * image_channels;

	// Tính lại stride (số byte mỗi hàng bao gồm padding)
	int current_padded_width_bytes;
	if (imageType == GREY) {
		current_padded_width_bytes = (cols + 2) * sizeof(uint8_t);
	}
	else {
		current_padded_width_bytes = (cols * 3 + 6) * sizeof(uint8_t);
	}

	// Con trỏ đến dữ liệu thực trong src
	uint8_t* local_data_start_ptr = src + (1 * current_padded_width_bytes) + (1 * image_channels);

	// Tạo MPI datatype mô tả block dữ liệu thực sự
	MPI_Datatype local_block_send_type;
	MPI_Type_vector(rows, cols * image_channels, current_padded_width_bytes, MPI_BYTE, &local_block_send_type);
	MPI_Type_commit(&local_block_send_type);

	// Chỉ tiến trình 0 cấp phát bộ nhớ ảnh kết quả và mảng đếm/offset
	int* recvcounts = NULL;
	int* displs = NULL;

	if (process_id == 0) {
		size_t final_image_total_bytes = (size_t)width * height * image_channels;
		final_image_buffer = (uint8_t*)malloc(final_image_total_bytes);
		recvcounts = (int*)malloc(num_processes * sizeof(int));
		displs = (int*)malloc(num_processes * sizeof(int));

		if (!final_image_buffer || !recvcounts || !displs) {
			fprintf(stderr, "Process 0: Failed to allocate memory.\n");
			MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
		}

		// Tính recvcounts và displs
		for (int p_rank = 0; p_rank < num_processes; ++p_rank) {
			int p_local_rows = height / row_div;
			int p_local_cols = width / col_div;
			recvcounts[p_rank] = p_local_rows * p_local_cols * image_channels;

			int p_start_row_global = (p_rank / col_div) * p_local_rows;
			int p_start_col_global = (p_rank % col_div) * p_local_cols;
			displs[p_rank] = ((size_t)p_start_row_global * width + p_start_col_global) * image_channels;
		}

		// ✅ Kiểm tra tổng số byte nhận được có đúng không
		size_t total_received_bytes = 0;
		for (int i = 0; i < num_processes; ++i) {
			total_received_bytes += recvcounts[i];
		}
		if (total_received_bytes != final_image_total_bytes) {
			fprintf(stderr, "Process 0: Mismatch in total received bytes (%zu != %zu)\n",
				total_received_bytes, final_image_total_bytes);
			MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
		}
		else {
			// ✅ Thêm dòng này để in số byte thu được
			printf("Process 0: Total bytes received: %zu bytes (expected: %zu bytes)\n",
				total_received_bytes, final_image_total_bytes);
		}
	}

	// Thu dữ liệu từ các tiến trình
	MPI_Gatherv(local_data_start_ptr, 1, local_block_send_type,
		final_image_buffer, recvcounts, displs, MPI_BYTE,
		0, MPI_COMM_WORLD);

	// Giải phóng kiểu dữ liệu MPI
	MPI_Type_free(&local_block_send_type);

	// Ghi file PNG tại tiến trình 0
	if (process_id == 0) {
		char png_filename[512];
		char* image_basename = strrchr(image, '\\');
		if (!image_basename) image_basename = strrchr(image, '/');
		image_basename = image_basename ? image_basename + 1 : image;

		snprintf(png_filename, sizeof(png_filename), "blur_%s.png", image_basename);
		int final_image_stride_bytes = width * image_channels;

		printf("Process 0: Writing PNG file: %s (%dx%d, channels: %d)\n", png_filename, width, height, image_channels);
		int success = stbi_write_png(png_filename, width, height, image_channels, final_image_buffer, final_image_stride_bytes);

		if (!success) {
			fprintf(stderr, "Process 0: Failed to write PNG file %s\n", png_filename);
		}
		else {
			printf("Process 0: Successfully wrote PNG file %s\n", png_filename);
		}
	...
	// --- KẾT THÚC KHỐI CODE THU THẬP DỮ LIỆU VÀ GHI FILE PNG ---
